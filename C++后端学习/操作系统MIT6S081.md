...
- 这些文件提供的函数（接口interfaces）都定义在文件kernel/defs.h中
...
- xv6中一个进程的**虚拟地址virtual address布局**：
  ...
  - 有一系列因素限制一个进程的最大虚拟地址：[MAXVA](https://github.com/mit-pdos/xv6-riscv/blob/riscv//kernel/riscv.h#L363)
    - RISC-V指令集中指针是64位的
  ...
  - 在虚拟地址空间的顶端(末尾)，xv6保留了2页page分别对应于trampoline和trapframe
    ...
    - trapframe：是一个结构体，用于保存用户态中断或异常发生时的寄存器值。

- Xv6内核对每一个进程设置了很多内核状态片段(piece of kernel state)，保存在一个**结构体[struct proc](https://github.com/mit-pdos/xv6-riscv/blob/riscv//kernel/proc.h#L86)**
...
2. **在machine mode下， CPU会在[_entry](https://github.com/mit-pdos/xv6-riscv/blob/riscv//kernel/entry.S#L7)处执行xv6操作系统：为xv6内核创建栈**

   - 为内核创建栈：xv6会为一个初始栈[stack0](https://github.com/mit-pdos/xv6-riscv/blob/riscv//kernel/start.c#L11)申请空间，然后在`_entry`的代码会将这个栈的指针stack pointer register `sp`加载到地址`stack0+4096`，也就是栈的顶部（因为RISC-V中的栈grow down）
   - 运行C代码：有了栈以后，Xv6内核就可以用[start](https://github.com/mit-pdos/xv6-riscv/blob/riscv//kernel/start.c#L21)函数调用C语言代码
...
4. **创建进程，并执行第一个系统调用exec()**
   ...
   - 在[main函数](https://github.com/mit-pdos/xv6-riscv/blob/riscv//kernel/main.c#L11)初始化了一些设备和子系统后，它会通过调用[userinit](https://github.com/mit-pdos/xv6-riscv/blob/riscv//kernel/proc.c#L226)来创建第一个进程。
   - 这个进程会执行RISC-V架构本身写好的一个小程序，用来执行Xv6系统的第一个系统调用：**exec()**
     1.  小程序[initcode.S](https://github.com/mit-pdos/xv6-riscv/blob/riscv//user/initcode.S#L3)会加载系统调用`exec`的数字代号：[SYS_EXEC](https://github.com/mit-pdos/xv6-riscv/blob/riscv//kernel/syscall.h#L8)到寄存器`a7`。
     2. 然后使用指令集的ecall指令来重新进入内核
     3. 内核通过[system call table](https://github.com/mit-pdos/xv6-riscv/blob/riscv//kernel/syscall.c#L108)将寄存器`a7`中保存的系统调用数字代号`SYS_EXEC`映射到`sys_exec`。
...
5. **启动初始化程序init，进入它的user mode**
   ...
   - [init](https://github.com/mit-pdos/xv6-riscv/blob/riscv//user/init.c#L15)会创建一个新的console终端，这个终端console会以文件描述符0,1,2的形式打开(因为linux系统中一切都是文件)，最后会在console中启动一个命令行解释器shell(用于用户在该终端console中输入命令)。
