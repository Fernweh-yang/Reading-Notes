...
1. 切换到分支，保存修改记录
   ...
   git switch xxx
   git add .
   git commit -m "change before rebase/merge"
   ```
...
3. 将main分支合并到xxx分支
   ...
   1. `merge`：将main的提交合并到当前分支，并创建一个新的合并提交

      ```
      git merge origin/main
      ```

   2. `rebase`：将当前分支的提交应用到`main`分支的最新提交值上，这样可以保持历史的线型
...
1. `git status`查看有哪些文件冲突了

2. 打开有冲突的文件，看到类似下面的标记：
   ...
   - `<<<<<<< HEAD` 和 `=======` 之间的部分是你当前分支的更改。
   - `=======` 和 `>>>>>>> branch_name` 之间的部分是另一个分支（例如 `origin/master`）的更改。

3. vim打开文件的话，就手动修改:

   - **保留自己的更改**： 删除 `=======` 和之后的部分，保留 `<<<<<<< HEAD` 和 `=======` 之间的内容。
   - **保留其他分支的更改**： 删除 `<<<<<<< HEAD` 和 `=======` 之间的部分，保留 `=======` 和 `>>>>>>> branch_name` 之间的内容。

4. vscode打开文件的话，除了手动修改，还可以直接可视化选择保留哪一个
