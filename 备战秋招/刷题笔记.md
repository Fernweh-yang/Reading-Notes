...
- 题目：[518. Coin Change II](https://leetcode.com/problems/coin-change-ii/)
...
- 思路：
  ...
  2. 递推公式
     ...
     因为定义为有几种方法，而不是最大值，所以这里的递推公式应该是所有dp[j-coins[i]]之和。参考13目标和

     `dp[j] += dp[j-coins[i]]`

  3. 初始化

     为了递归dp[0]需要初始化为1，否则所有的递归和都是0

     其他非0的dp[j]都定义为0，这样累计加dp[j - coins[i]]的时候才不会影响真正的dp[j]

  4. 遍历顺序

     - 因为是组合数(不强调物品之间的顺序)：所以先遍历硬币，再遍历背包。

       此时先放1，再放5后，得到的方法值是{1,5}而没有{5,1}

     - 如果是排列数(强调背包中物品之间的顺序)：就要先遍历背包，再遍历硬币。

       此时对于每个背包，都会遍历一次硬币，所以还有{1,5}和{5,1}两种情况
...
- 题目：[377. Combination Sum IV](https://leetcode.com/problems/combination-sum-iv/)
...
- 题目：[70. Climbing Stairs](https://leetcode.com/problems/climbing-stairs/description/)
  ...
  上面就是3.题，那里用了最直接的动规方法。现在稍微修改成如下
...
- 题目：[322. Coin Change](https://leetcode.com/problems/coin-change)

  You are given an integer array `coins` representing coins of different denominations面额 and an integer `amount` representing a total amount of money.
...
- 思路
  ...
  3. 初始化
     ...
     其余dp[j]要足够大，因为最多全都由amount个1组成，那把所有值设成amount+1就好了

  4. 遍历顺序

     由于没说是排列还是组合，所以对于完全背包而言，先遍历物品还是先遍历背包无所谓

  5. 举例

     对于coins = [1,2,5], amount =4

     ```
     初始化：dp[0]=0 dp[j]=amount+1=5
     j=1 dp[1]=dp[1-1]+1=1
     j=2 dp[2]=dp[2-1]+1=2
     	dp[2]=dp[2-2]+1=1 => dp[2] = 1
     j=3 dp[3]=dp[3-1]+1=2
     	dp[3]=dp[3-2]+1=2 => dp[3] = 2
     j=4 dp[4]=dp[4-1]+1=3
     	dp[4]=dp[4-2]+1=2 => dp[4] = 2
     ```
...
- 题目：[279. Perfect Squares](https://leetcode.com/problems/perfect-squares)
...
- 思路
  ...
  1. dp数组的意义
     ...
     dp[j]：组成j所需的最少完美平方数个数

  2. 递推公式

     先找出所有可能的perfect square的数组vec。
  ...
  4. 遍历顺序
     ...
     因为是完全背包，也不要求组合还是排列，所以任意先背包还是物品
...
- 题目：[139. Word Break](https://leetcode.com/problems/word-break/)
...
- 思路
  ...
  1. dp数组的含义
     ...
     dp[i]：字符串s长度为i是，dp[i]若无true，则表示其可以拆分为一个或多个在字典中出现的单词

  2. 递推公式

     j < i, 用s.substr(i-j)表示单词

     如果[j,i]这个区间的子串出现在字典中&&dp[j]为true，那么dp[i]是true
  ...
  4. 遍历顺序
     ...
     本题是排列数（先放哪一个物品都可以），所以一定是先遍历背包，再遍历物品。

     

     不能先遍历物品再遍历背包：这种方法只适合aaabb这种形式的字符串，不适合abab形式的字符串

     - 参考例子s = "applepenapple", wordDict = ["apple", "pen"]:
...
- 题目: [198. House Robber](https://leetcode.com/problems/house-robber)
...
- 思路
  ...
  容易想到直接一个隔着一个搜不肯定是最大的了吗，但[2,1,1,2]就需要跳过2个来偷。

  - dp数组的意义

    dp[i]：前i个包括i个房子，最多可以偷窃dp[i]金额的钱
...
- 题目：[213. House Robber II](https://leetcode.com/problems/house-robber-ii)
...
- 思路
  ...
  和23的打家劫舍类似，只是这里成环了，需要考虑2种情况：1. 包含头，不考虑尾  2.包含尾，不考虑头
  ...
  2. 递推公式
     ...
     假设有size个房子，分别递推i=0到size-2（不包含尾），和递推i=1到size-1（不包含头），然后选更大的那个
...
- 代码
  ...
  public:
      int rob(vector<int>& nums) {
          ...
          vector<int> num1(nums.begin(),nums.end()-1);	// 情况1 不考虑尾
          vector<int> num2(nums.begin()+1,nums.end());	// 情况2 不考虑头
          int cash1 = Rob(num1);
          int cash2 = Rob(num2);
...
- 题目：[337. House Robber III](https://leetcode.com/problems/house-robber-iii/)
...
- 思路：
  ...
  **这道题目算是树形dp的入门题目，因为是在树上进行状态转移，我们在讲解二叉树的时候说过递归三部曲，那么下面我以递归三部曲为框架，其中融合动规五部曲的内容来进行讲解**。
  ...
  1. 确定递归函数的参数和返回值
     ...
     要求返回当前节点偷/不偷两种情况下得到的金钱，所以返回值应该是一个长度为2的数组`vector<int> dp(2,0)`
     ...
     可见返回的数组也就是动态规划的dp数组：`vector<int> dp(2,0)`
...
- 代码
  ...
  public:
      vector<int> robTree(TreeNode* cur){
          // 1.返回值dp的数组的含义：dp[0]不偷当前节点得到的钱，dp[1]偷当前节点得到的钱
          vector<int> dp(2,0);
...
- 思路
  ...
  因为本题是任意买卖次数又只有1种产品，所以可以直接双指针遍历，右指针比左指针大就做这一对交易。
  ...
  2. 递推公式
     ...
     - 当属于`dp[i][0]`当天持有的情况
       ...
       - 第i天买入股票，所得现金就是昨天不持有股票的所得现金减去 今天的股票价格 即：`dp[i - 1][1]` - prices[i]
         ...
         **注意：**和26题不同，这里不是只有-prices[i]了，因为本题不仅可以买卖一次，还要考虑之前买卖得到的利益情况
       
     - 当属于`dp[i][1]`当天没有的情况
  
       - 第i-1天就没有股票，那么就保持现状，所有现金就是昨天没有股票的现金 即：`dp[i-1][1]`
       - 第i-1天卖掉股票，那么现在的现金就是当天卖掉的钱+昨天持有股票时的钱 即：`prices[i]+dp[i-1][0]`
  
  3. 初始化
  
     `dp[0][0]=-prices[0]`：第一天持有，那么花了就是第一天的价格
  
     `dp[0][1]=0`：第一天不持有，那么就是0
...
- 题目：[123. Best Time to Buy and Sell Stock III](https://leetcode.com/problems/best-time-to-buy-and-sell-stock-iii/)
...
- 思路：
  ...
  1. dp数组的含义
     ...
     所以`dp[i][j]`中 i表示第i天，j为 [0 - 4] 五个状态，`dp[i][j]`表示第i天状态j所剩最大现金。
  ...
  2. 递推公式
     ...
     2. 对于状态1：第一次持有股票
        ...
        根据dp数组的定义：`dp[i][1]=max(dp[i-1][0]-prices[i],dp[i-1][1])`

     3. 对于状态2：第一次不持有股票

        有两个可能的操作：

        - 操作一：第i天卖出了股票，那么`dp[i][2]=dp[i-1][1]+prices[i]`
        - 操作二：第i天没有操作，而是沿用前一天不持有的状态，即`dp[i][2]=dp[i-1][2]`

        根据数组的定义：`dp[i][2]=max(dp[i-1][1]+prices[i],dp[i-1][2])`

     4. 对于状态3：第二次持有股票

        - 操作一：第i天买入了股票，那么`dp[i][3]=dp[i-1][2]-prices[i]`
        - 操作二：第i天没有操作，而是沿用前一天买入的状态`dp[i][3]=dp[i-1][3]`

        根据数组的定义：`dp[i][3]=max(dp[i-1][2]-prices[i],dp[i-1][3])`

     5. 对于状态4：第二次卖出股票

        - 操作一：第i天卖出了股票，那么`dp[i][4]=dp[i-1][3]+prices[i]`
        - 操作二：第i天没有操作，而是沿用前一天不持有的状态，即`dp[i][4]=dp[i-1][4]`

        根据数组的定义：`dp[i][4]=max(dp[i-1][3]+prices[i],dp[i-1][4])`

  3. 初始化

     对于状态0：`dp[0][0]=0`

     对于状态1：`dp[0][1]=-prices[0]`

     对于状态2：`dp[0][2]=0` 可以理解为当天买入并卖出

     对于状态3：`dp[0][3]=-prices[0]` 可以理解为当天买入卖出后再买入

     对于状态4：`dp[0][4]=0`可以一天执行了2次买入卖出
...
- 代码
  ...
  public:
      int maxProfit(vector<int>& prices) {
          int size = prices.size();
          if(size < 1) return 0;
          vector<vector<int>> dp(size,vector(5,0));
          dp[0][0] = 0;
          dp[0][1] = -prices[0];
          dp[0][2] = 0;
          dp[0][3] = -prices[0];
...
- 题目：[188. Best Time to Buy and Sell Stock IV](https://leetcode.com/problems/best-time-to-buy-and-sell-stock-iv/)
...
- 思路：
  ...
  1. dp数组的意义
     ...
     `dp[i][j]`表示第i天的第j个状态下最多的现金。

     有`j=2k+1`个状态: $n\in[1,3,5,...,2k-1]$
  ...
  2. 递推公式
     ...
     - 对于奇数状态 n：
       ...
       - 当天买入：`dp[i][n]=dp[i-1][n-1]-prices[i]`
       - 当天没操作：`dp[i][n]=dp[i-1][n]`

       根据dp数组的含义：`dp[i][n]=max(dp[i-1][n-1]-prices[i],dp[i-1][n])`

     - 对于偶数状态n+1:

       有两种可能的情况

       - 当天卖出:`dp[i][n+1]=dp[i-1][n+1]+prices[i]`
       - 当天无操作：`dp[i][n+1]=dp[i-1][n+1]`

       根据dp数组的含义：`dp[i][n+1]=max(dp[i-1][n+1]+prices[i],dp[i-1][n+1])`

  3. 初始化

     - `dp[0][0]=0`
     - `dp[0][n]=-prices[0]`: 可以认为第0天发生了n-1次买卖，然后又发生了第n次买
     - `dp[0][n+1]=0`：可以认为第0天发生了n+1次买卖
...
- 代码
  ...
  public:
      int maxProfit(int k, vector<int>& prices) {
          int size = prices.size();
          if(size<1) return 0;
          vector<vector<int>> dp(size,vector(2*k+1,0));
          // 注意遍历时i+=2
          ...
          for(int i=1;i<size;i++){
              ...
              for(int n=1;n<2*k;n+=2){
                  dp[i][n] = max(dp[i-1][n-1]-prices[i],dp[i-1][n]);
                  dp[i][n+1]=max(dp[i-1][n]+prices[i],dp[i-1][n+1]);
              }
          }
...
- 题目：[309. Best Time to Buy and Sell Stock with Cooldown](https://leetcode.com/problems/best-time-to-buy-and-sell-stock-with-cooldown/)
...
- 思路：
  ...
  1. dp数组的意义
     ...
     `dp[i][j]`表示在第i天j状况下能得到的最多钱

     有4个状态

     0. 持有：
        - 当天买入。或者昨天冷冻期买入。或者昨天是未持有状态1买入
        - 之前就持有
     1. 未持有：两天前卖出，即度过了冷冻期。或者昨天是冷冻期
     2. 未持有：当天卖出
     3. 冷冻期
  ...
  2. 递推公式
     ...
     0. 持有：
        ...
        `dp[i][0]=max(dp[i-1][3]-prices[i],dp[i-1][1]-prices[i],dp[i-1][0])`
...
- 代码
  ...
  public:
      int maxProfit(vector<int>& prices) {
          int size = prices.size();
          if(size < 1) return 0;
          vector<vector<int>> dp(size,vector(4,0));
          dp[0][0]=-prices[0];
          for(int i=1; i<size; i++){
...
- 题目：[714. Best Time to Buy and Sell Stock with Transaction Fee](https://leetcode.com/problems/best-time-to-buy-and-sell-stock-with-transaction-fee/)
...
- 思路
  ...
  1. dp数组的意义
     ...
     `dp[i][j]`第i天j状态下最多能赚到的钱。

     有2种状态

     0. 持有股票：当天购入，或之前就购入了
     1. 不持有股票：当天卖出，或之前就没持有

  2. 递推公式

     统一：购入股票时交fee,卖出时不用交fee
...
- 题目：[300. Longest Increasing Subsequence](https://leetcode.com/problems/longest-increasing-subsequence/)
...
- 思路
  ...
  1. dp数组的含义
     ...
     `dp[i]`：前i个字符(包含nums[i])最长上升子序列

  2. 递推公式

     位置i的最长升序子序列等于j从0到i-1各个位置的最长升序子序列 + 1 的最大值。因为是包含nums[i]的，所以要+1
...
- 题目：[674. Longest Continuous Increasing Subsequence](https://leetcode.com/problems/longest-continuous-increasing-subsequence/)
...
- 思路
  ...
  2. 递推公式
     ...
     如果nums[i-1] < nums[i]，那么dp[i]=dp[i-1]+1
...
- 思路：
  ...
  本题要求**重复子数组是连续**的，基本思路就是2个指针遍历2个数组找出第一个数组每一个元素开始对应的最长重复子数组，储存的方式借dp的思想表现了出来
...
- 思路：
  ...
  和上面的34相比，这里不要求是连续的了，但要有相对顺序。所以不像上面元素相等的情况如果断了就重新计数，这边断了是继续计数下去的

  1. dp数组的意义

     `dp[i][j]`表示[0, i - 1]的字符串text1与长度为[0, j - 1]的字符串text2的最长公共子序列个数
...
- 题目：[1035. Uncrossed Lines](https://leetcode.com/problems/uncrossed-lines/)
...
- 思路
  ...
  **本题说是求绘制的最大连线数，其实就是求两个字符串的最长公共子序列的长度！**
  ...
  1. dp数组的含义
     ...
     `dp[i][j]`表示nums1的[0,i-1]子数组和nums2的[0,j-1]的子数组的最长公共子序列
...
- 思路
  ...
  2. 递推公式
     ...
     `dp[i]=max(dp[i-1]+nums[i],nums[i])`：即要么将当前值加入序列和，要么从当前值开始从头计算
...
- 思路:
  ...
  1. dp数组的定义
     ...
     `dp[i][j]`：字符串s的前i-1位，和字符串t的前j-1位的最大子序列和

  2. 递推公式

     如果s[i-1]==t[j-1]，那么`dp[i][j]=dp[i-1][j-1]+1`

     如果`s[i-1]!=t[j-1]`，那么`dp[i][j]=max(dp[i-1][j],dp[i][j-1])`
...
- 题目：[115. Distinct Subsequences](https://leetcode.com/problems/distinct-subsequences/)
...
- 思路
  ...
  1. dp数组的意义
     ...
     `dp[i][j]`：以i-1为结尾的s子序列中出现t的前j-1个字符组成的字符串的个数

  2. 递推公式

     s[i-1]==t[j-1]时：

     - 使用s[i-1]来匹配，那么个数就是`dp[i-1][j-1]`

       即不需要考虑当前s子串和t子串的最后一位字母，所以只需要 `dp[i-1][j-1]`。

     - 不使用s[i-1]来匹配，那么个数是`dp[i-1][j]`

       相当于模拟s中删除这个元素i-1

     => `dp[i][j]=dp[i-1][j-1] + dp[i-1][j]`将所有的情况加起来

     s[i-1]!=t[j-1]时：

     - 相当于模拟s中删除这个元素i-1，所以`dp[i][j]=dp[i-1][j]`
...
- 代码
  ...
  public:
      int numDistinct(string s, string t) {
          int size1 = s.size();
          int size2 = t.size();
          vector<vector<uint64_t>> dp(size1+1,vector<uint64_t>(size2+1));
...
- 题目：[72. Edit Distance](https://leetcode.com/problems/edit-distance/)
...
- 思路
  ...
  1. dp数组的意义
     ...
     dp[i][j]表示[0,i-1]的word1和[0,j-1]的word2的最小编辑距离

  2. 递推公式

     - 如果word[i-1]==word2[j-1] 那么`dp[i][j] = dp[i-1`][j-1]
     - 如果word[i-1]==word2[j-1] 那么`dp[i][j] = min(dp[i-1][j-1]，dp[i-1][j],dp[i][j-1])+1` 分别对应：替换，wrod1删除，word2删除(也可以理解为word1插入)           

  3. 初始化

     先全部初始化为0

     对于`dp[i][0]`，显然要删掉所有的i，所以定义为i

     对于`dp[0][j]`，显然要删掉所有的j，所以定义为j
...
- 题目：[647. Palindromic Substrings](https://leetcode.com/problems/palindromic-substrings/)
...
- 思路
  ...
  1. 确定dp数组及其下标的意义
     ...
     bool类型的`dp[i][j]`：表示区间范围`[i,j]`的子串是否为回文子串，如果是则为true

  2. 递推公式

     如果字符串字符`s[i]`与`s[j]`不相等，`dp[i][j]=false`

     如果字符串字符`s[i]`与`s[j]`相等，有三种情况：

     1. 下标i和j相等，即指向同一个字符，那么是回文`dp[i][j]=true`
     2. 下标i和j的值差1，如aa，那么是回文`dp[i][j]=true`
     3. 下标i和j值的差>1，那么就要看`dp[i+1][j-1]`是否为true了，如果是就是true，否则false
  ...
  4. 确定遍历顺序
     ...
     可以发现`dp[i+1][j-1]`要在`dp[i][j]`之前计算，所以一定要从下到上，从左到右遍历，这样保证`dp[i + 1][j - 1]`都是经过计算的。i从s的最后一位开始。
...
- 题目：[516.Longest Palindromic Subsequence](https://leetcode.com/problems/longest-palindromic-subsequence/description/)
...
- 思路
  ...
  与49.回文子串要连续的不同，回文子序列可以不是连续的

  1. dp数组的意义

     `dp[i][j]`：字符串s在[i,j]范围内最长的回文子序列长度

  2. 递推公式

     如果s[i]和s[j]相同，那么`dp[i][j] = dp[i + 1][j - 1] + 2`

     如果s[i]和s[j]不同，需要分别加入s[i]、s[j]看看哪一个可以组成最长的回文子序列：即`dp[i][j] = max(dp[i + 1][j], dp[i][j - 1]);`
  ...
  4. 遍历顺序
     ...
     从递归公式中，可以看出，`dp[i][j]` 依赖于 `dp[i + 1][j - 1]` ，`dp[i + 1][j]` 和` dp[i][j - 1]`，即：

     ![](https://raw.githubusercontent.com/Fernweh-yang/ImageHosting/main/img/%E6%9C%80%E5%B0%8F%E5%9B%9E%E6%96%87%E5%AD%90%E4%B8%B2.png)
