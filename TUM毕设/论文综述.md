...
- 主要思想：
  ...
  ![image-20231108203252816](https://raw.githubusercontent.com/Fernweh-yang/ImageHosting/main/img/model_selection_pipeline.png)

  <center style="color:#C125C0C0">图1：Pipeline
  </center>

  语义分割的方法可以分为两类：一类追求准确的结果但低语义推断inference速度(ENet, ICNet)，另一类是牺牲准确性追求速度(PSPNet, DeepLab)

  基于此，先用速度快的语义分割模型，然后计算entropy map作为评判不确定度的标准。最后再对不确定度低的区域用准确度高的语义分割模型

- 如何建立不确定性：

  图片经过一个分割网络$F$，得到每个像素的logit vector $y$，对这个logits $y$施加softmax运算得到预测的分类概率。最后用entropy map作为不确定度测量。有了softmax z之后的预测概率，entropy map由如下公式计算：
  $$
  H[z]=-\sum_{i=1}^Kz_i*log_2(z_i)
  $$
  背后思想是：softmax给出的第1，2高的分类概率之间的差值如果很大，说明不确定性越小。
...
- 主要思想：
  ...
  利robust training methods（本文用的compensation learning）和 不确定性估计 来避免在数据标注时产生新的误差。（区别于其他方法：利用不确定性估计来训练更稳定的模型对抗噪声）

- pipeline

  ![image-20231109172848720](https://raw.githubusercontent.com/Fernweh-yang/ImageHosting/ad1a4dd3acde357e145cd664807d452d12ecc436/img/Compensation%20Learning%20in%20Semantic%20Segmentation.png)

  <center style="color:#C125C0C0">图1：本文pipeline (NEC)</center>

   用一个补偿矩阵B学习真值和局部重要因子$\beta$一起识别有噪声的区域，再结合卷积神经网络$F$学到的语义分割$l_x$ ，来优化损失函数。

- 如何表示不确定性

  这个局部重要因子$\beta$同样是由卷积网络$F$学习得到的。



### Efficient Uncertainty Estimation in Semantic Segmentation via Distillation

- 这里的Distillation蒸馏是什么意思

  较大模型的“知识”被提炼或压缩compress到一个简单紧凑compact的模型，使得小模型能够学习和模仿大模型的复杂决策过程和输出。

  本文中小模型不仅会学习大模型的预测，还会学习这些预测在模型中的概率分布，使得小模型能量化他们的不确定度

- pipeline

  ![image-20231110171956172](https://raw.githubusercontent.com/Fernweh-yang/ImageHosting/aa5a02eea5ef5f49d93898392e06d6f146294ac1/img/Uncertainty%20Distillation%20Pipeline.png)

  <center style="color:#C125C0C0">图1：本文pipeline</center>

  输入图像$x_i$给多个模型组合起来的复杂模型$f_t$和简单模型$f_s$，复杂模型的每个子模型算出F个语义分割图，并输出他们的mean $M_i$ 和 standard deviation $\Sigma_i$ 。简单模型会计算一个不确定度$x^*_i$和语义分割$x'_{f_si}$。他们还有真值$y_i$之间会算出3个损失函数用于训练简单模型：分类损失$L_{cks}$，蒸馏损失$L_{dist}$，不确定度损失$L_{unc}$

- 如何得到不确定度

  由一个uncertainty head算出的uncertainty map

  > Each head comprises a 3 × 3 conv-BN-ReLU block with 256 output channels, followed by a 1 × 1 convolution with C output channels, and a sigmoid function bounds uncertainty outputs between 0 and 1, while the softmax function (2) is applied to prediction outputs.

  根据论文描述，这个uncertainty head就是一个sofmax

### Hyperbolic Image Segmentation

- 主要思想

  有点玄学。。使用**超几何流形hyperbolic manifolds**来进行语义分割 并 测量语义分割边界处的不确定度。

- 如何得到不确定度

  区别于传统贝叶斯神经网络，本文是通过测量Poincare ball原点的$l_2$范数来得到不确定度。

  > Poincaré ball 是超几何空间的一种表示方式，通常用于可视化和计算。在这个模型中，超几何空间被表示为一个单位半径的球形空间。这个球被嵌入一个更高维的欧几里得空间中，但在数学处理时，只考虑球的表面或内部。在 Poincaré ball 模型中，可以研究超几何空间中的几何特性，比如点、线、角度和曲率等。
...
- 主要思想：
  ...
  基于深度学习，通过incorporate合并feature position uncertainty特征位置不确定度到视觉里程计，来提升帧与帧之间的相对位姿。
  
- 主要贡献

  1. 提出了对称symmetric版本的**PNEC**，可以更准确的建模带有不确定特征位姿的相对位姿估计的几何geometry。

  2. 通过学习differentiable nonlinear least squares (**DNLS**)构建的feature position uncertainty来最小化相对位姿误差
  
     > 可微分非线性最小二乘（Differentiable Nonlinear Least Squares，DNLS）

- 方法概述：

  通过forward和backward两步来学习noise covariances
  
  - forward:
  
    即PNEC：协方差covariances被用在概率的位姿估计优化
  
  - backward:
  
    位姿误差的梯度通过优化反向传播到协方差

- **Probabilistic Normal Epipolar Constraint**（PNEC）

  PNEC是为了优化R,t

  - **不对称asymmetric版本的PNEC**详细见作者另一篇[论文](https://arxiv.org/abs/2204.02256)

    - 用处：

      给定2张图$I,I'$，假设第二张图特征点位置是不确定的情况下，估计出两图之间的相对位姿

    - Energy function:
      $$
      E(\mathbf{R},\mathbf{t})=\sum_i\frac{e_i^2}{\sigma_i^2}=\sum_i\frac{|\mathbf{t}^T(\mathbf{f}_i\times\mathbf{R}\mathbf{f}_i')|^2}{\mathbf{t}^T\hat{\mathbf{f}}_i\mathbf{R}\Sigma_i'\mathbf{R}^T\hat{\mathbf{f}}_i^T\mathbf{t}}\tag{1}
      $$

      - $e_i=|t^Tn_i|$：epipolar error

        NEC的概念来自ETHZ的[论文](chrome-extension://efaidnbmnnnibpcajpcglclefindmkaj/https://people.inf.ethz.ch/pomarc/pubs/KneipECCV12.pdf)

        ![image-20231107211347100](https://raw.githubusercontent.com/Fernweh-yang/ImageHosting/73377a131bcf9a36ada77e5cf019ca9cc138e733/img/NEC.png)

        <center style="color:#C125C0C0">图1：Geometry of the normal epipolar constraint (NEC)</center>

        基本思想是，法向量垂直于位移t，通过强制法向量的共面性coplanarity来估计旋转。可以看到位移t垂直于各个epipolar plane的法线，因此如果法线是无误的那么他们相乘$e_i=|t^Tn_i|=0$，否则就可以优化了。

        - $t$：位移

        - $\mathbf{n}_i=\mathbf{f}_i\times\mathbf{R}\mathbf{f}_i'$：一个表示epipolar plane的法向量normal vector

        - $\mathbf{f}_i$：unit bearing vector单位方向向量

          表示从相机远点指向特征点的方向，参考对极几何
        
      - $\sigma_i^2(\mathbf{R},\mathbf{t})=\mathbf{t}^T\hat{\mathbf{f}}_i\mathbf{R}\Sigma_i'\mathbf{R}^T\hat{\mathbf{f}}_i^Tt$ 高斯分布的方差
      
        - $\hat{x}$：戴帽子的变量表示向量x的skew-symmetric matric 
        - $\Sigma_i'$：第二张图的方向向量$\mathbf{f}_i'$的3D协方差矩阵，体现了第二张图特征点位置的不确定性

  - **使PNEC对称**

    - 用处：

      给定2张图$I,I'$，假设第一二张图特征点位置都是不确定的情况下，估计出两图之间的相对位姿

    - Energy Function
      $$
      E_s(\mathbf{R},\mathbf{t})=\sum_i\frac{e_i^2}{\sigma_{s,i}^2}=\sum_i\frac{|\mathbf{t}^T(\mathbf{f}_i\times\mathbf{R}\mathbf{f}_i')|^2}{\mathbf{t}^T((\hat{\mathbf{Rf_i'}})\mathbf{\Sigma_i}(\hat{\mathbf{Rf_i'}})^T+\hat{\mathbf{f}}_i\mathbf{R}\Sigma_i'\mathbf{R}^T\hat{\mathbf{f}}_i^T)\mathbf{t}}\tag{2}
      $$

      - $\Sigma_i,\Sigma_i'$：分别是第一张和第二张图的方向向量$\mathbf{f}_i,\mathbf{f}_i'$的3D协方差，体现了第一二章图特征点位置的不确定性
      
        根据作者另一篇[论文](https://arxiv.org/abs/2204.02256)，这两个3D协方差都是由各自图片的2D协方差$\Sigma_{2D,i}$（代表了特征点的不确定度）得到的，在那篇论文里这个2D协方差是假定已知的，在这篇论文里这个2D协方差貌似是靠UNet估计出来的。

  - PNEC的训练Scheme同样参见作者另一篇[论文](https://arxiv.org/abs/2204.02256)

- **用DNLS来学习协方差（不确定度）**

  是为了估计2D协方差$\Sigma_{2D}$（代表了特征点的不确定度）

  - 计算公式：
    $$
    \Sigma_{2D}=arg\ \mathop{min}_{\Sigma_{2D}}\ \mathcal{L} \tag{3}
    $$
    作者只考虑了旋转误差：
    $$
    \mathcal{L}(\widetilde{R},R;\Sigma_{2D})=e_{rot}=\widetilde{R}^TR\tag{4}
    $$
    ...
    - $R$：最小化2式得到：
      $$
      R=arg\ \mathop{min}_R\ E_s(R,t;\Sigma_{2D})\tag{5}
      $$

    为了学习得到$\Sigma_{2D}$，就要求损失函数对它的梯度函数：
    $$
    \frac{d\mathcal{L}}{d\Sigma_{2D}}=-\frac{\partial^2E_s}{\partial\Sigma_{2D}\partial\mathbf{R}^T}(\frac{\partial^2E_s}{\partial\mathbf{R}\partial\mathbf{R}^T})^{-1}\frac{e_{rot}}{\partial\mathbf{R}}\tag{6}
    $$

  - 对于监督学习supervised learning

    由于有真值，损失函数可以就是4式

  - 对于自监督学习self-Supervised learning

    由于没有真值（也是大部分情况），使用3张图来求cycle-consistency loss
    $$
    \mathcal{L}_{cycle}=\prod_{(i,j)\in\mathcal{P}}\mathbf{R}_{ij}\ \ ,\mathcal{P}=\{(1,2),(2,3),(3,1) \}\tag{7}
    $$
    另外再将PNEC中估计得到的$\mathbf{R}_{ij,NEC}$也考虑进这个损失函数
    $$
    \mathcal{L}_{another}=\sum_{(i,j)\in\mathcal{P}}\mathbf{R}_{ij}\mathbf{R}_{ij,NEC}^T\tag{8}
    $$
    总的自监督学习损失为：
    $$
    \mathcal{L}_{self}=\mathcal{L}_{cycle}+\lambda\mathcal{L}_{another}
    $$

- 整体结构：

  ![image-20231108021845008](https://raw.githubusercontent.com/Fernweh-yang/ImageHosting/main/img/keypoint_uncertainty_architecture.png)

  <center style="color:#C125C0C0">图2：keypoint_uncertainty_architecture</center>

  - 第一步：基于[UNet](https://arxiv.org/abs/1505.04597)得到每一个像素的不确定，结合关键点提取器得到的关键点，就得到了这些带有不确定性估计的特征点
  - 第二步：使用DNLS来优化估计位姿
  - 第三步：UNet由DNLS里的梯度反向传播后更新
