...
- **Probabilistic Normal Epipolar Constraint**（PNEC）
  ...
  - **使PNEC对称**
    ...
    - Energy Function
      $$
      E(\mathbf{R},\mathbf{t})=\sum_i\frac{e_i^2}{\sigma_{s,i}^2}=\sum_i\frac{|\mathbf{t}^T(\mathbf{f}_i\times\mathbf{R}\mathbf{f}_i')|^2}{\mathbf{t}^T((\hat{\mathbf{Rf_i'}})\mathbf{\Sigma_i}(\hat{\mathbf{Rf_i'}})^T+\hat{\mathbf{f}}_i\mathbf{R}\Sigma_i'\mathbf{R}^T\hat{\mathbf{f}}_i^T)\mathbf{t}}\tag{2}
      $$
...
- **用DNLS来学习协方差（不确定度）**
  ...
  见论文，参考作者另一篇[论文](https://arxiv.org/abs/2204.02256)训练PNEC的Scheme可能有用
